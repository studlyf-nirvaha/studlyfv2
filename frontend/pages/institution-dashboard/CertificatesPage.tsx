...
interface CertificateRecord {
  ...
  rank?: number; score?: number; user_id?: string; participant_id?: string;
}
                          ...
                          <tr key={c._id} className="border-b border-slate-100 hover:bg-slate-50">
                            ...
                            <td className="py-3 px-4">
                              <div className="flex items-center justify-center space-x-2 text-indigo-600">
                                <span title="Preview Certificate"><Eye className="w-4 h-4 cursor-pointer hover:text-indigo-800" onClick={() => setSelectedCertificate(c)} /></span>
                                {!isPending && <><span title="Download PDF"><Download className="w-4 h-4 cursor-pointer hover:text-indigo-800" onClick={() => alert('Download certificate: ' + (c.certificate_id || c._id))} /></span>
                                <span title="Email Certificate"><Mail className="w-4 h-4 cursor-pointer hover:text-indigo-800" onClick={() => alert('Send email to: ' + c.email)} /></span></>}
              ...
              <div className="space-y-2">
                ...
                <textarea value={typeof formData.rule_config === 'object' ? JSON.stringify(formData.rule_config, null, 2) : formData.rule_config}
                  onChange={(e) => {
                    ...
                    catch { /* keep previous value if JSON is invalid */ }
                  }}
